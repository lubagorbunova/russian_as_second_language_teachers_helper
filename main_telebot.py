...
while started:
    if requestQueue.qsize() > 0:
        ...
        if request.text == '/start':
            ...
            responseQueue.put(TelebotResponse(chat=request.chat, text=ui_texts['start']))
            responseQueue.put(TelebotResponse(chat=request.chat, text=ui_texts['choose_text'], commands=my_text_or_base))
